...
const int nDesTimes = 5;
double Tesc_desired_all[nDesTimes] = {26.0, 57.0, 122.0, 262.0, 565.0}; 
...
int main(int argc, char *argv[]){
    ...
    ofstream avgInfoOut;
    avgInfoOut.open("avgStatistics_nLevel_1_"+retTruncDbl( round(1/D),0 )+".txt", ofstream::out | ofstream::trunc);

    cout << "computing paths for noise level D = 1/"<< retTruncDbl( round(1/D),0 ) << endl; 
    cout << "number of threads used " << nThreads << endl;
    
    // compute paths for each desired escape time
    for ( int desTimeCount = 0; desTimeCount < nDesTimes; desTimeCount++){
     
        lvlStTime = time(NULL);

        // set desired escape time
        Tesc_desired = Tesc_desired_all[desTimeCount];
        
        cout << "\tcomputing paths for t_desired (" << desTimeCount+1 << "/" << nDesTimes << ") : "<< Tesc_desired << "s" << endl;
